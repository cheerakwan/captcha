...
func TestCaptchaPatternTwoLeftOneOperatorTwoRightOneShouldReturnOneMinus1(t *testing.T) {
	result := Captcha(2, 1, 2, 1)
	expected := "one - 1"
	if result != expected {
		t.Errorf("Expected %q, But result is %q", expected, result)
	}
...
func TestCaptchaPatternOneLeftOneOperatorThreeRightTwoShouldReturn1MultiplyTwo(t *testing.T) {
	result := Captcha(1, 1, 3, 2)
	expected := "1 * two"
	if result != expected {
		t.Errorf("Expected %q, But result is %q", expected, result)
	}
